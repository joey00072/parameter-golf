...
fp16 embed + late-K passthrough + sliding window eval + par layer bigram.
"""
...
class BigramHashEmbedding(nn.Module):
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int,
                 hash_a: int = 36313, hash_b: int = 27191):
        super().__init__()
...
# Large primes for per-layer bigram hashing — different collision pattern at each depth
_BIGLU_PRIMES = [
    (104729, 86351),
    (224737, 198491),
    (350377, 324653),
...
class BigLU(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, bigram_vocab_size: int, layer_idx: int = 0, biglu_mlp_mult: float = 0.5):
        super().__init__()
        hidden = int(biglu_mlp_mult * dim)
        self.up = CastedLinear(dim, hidden, bias=False)
        self.down = CastedLinear(hidden, dim, bias=False)
        self.down._zero_init = True
        hash_a, hash_b = _BIGLU_PRIMES[layer_idx % len(_BIGLU_PRIMES)]
        self.bigram = BigramHashEmbedding(bigram_vocab_size, hidden, hidden, hash_a=hash_a, hash_b=hash_b)
...
class Block(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        biglu_bigram_vocab_size: int = 8192,
        biglu_mlp_mult: float = 0.5,
        rope_dims: int = 0,
        layer_idx: int = 0,
        ln_scale: bool = False,
    ):
        ...
        self.mlp = BigLU(dim, mlp_mult, biglu_bigram_vocab_size, layer_idx=layer_idx, biglu_mlp_mult=biglu_mlp_mult)
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())
        self.ln_scale_factor = 1.0 / math.sqrt(layer_idx + 1) if ln_scale else 1.0

    def forward(self, x: Tensor, x0: Tensor, token_ids: Tensor) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        s = self.ln_scale_factor
        attn_out = self.attn(self.attn_norm(x) * s)
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x) * s, token_ids)
        return x
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    biglu_bigram_vocab_size=biglu_bigram_vocab_size,
                    biglu_mlp_mult=biglu_mlp_mult,
                    rope_dims=rope_dims,
                    layer_idx=i,
                    ln_scale=ln_scale,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        biglu_bigram_vocab_size=args.biglu_bigram_vocab_size,
        biglu_mlp_mult=args.biglu_mlp_mult,
    ...
    optimizer_biglu = torch.optim.AdamW(
        [{"params": biglu_adam_params, "lr": biglu_lr, "base_lr": biglu_lr}],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        weight_decay=args.adam_wd,
        fused=True,
    )
    optimizers: list[torch.optim.Optimizer] = [optimizer_tok, optimizer_muon, optimizer_scalar, optimizer_biglu]
    ...
    eval_model = GPT(
        ...
        biglu_bigram_vocab_size=args.biglu_bigram_vocab_size, biglu_mlp_mult=args.biglu_mlp_mult,
        xsa_last_n=args.xsa_last_n,
        rope_dims=args.rope_dims,
        ln_scale=args.ln_scale,
    ).to(device).bfloat16()
