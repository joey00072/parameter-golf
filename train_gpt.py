...
class Block(nn.Module):
    def __init__(
        ...
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim), torch.zeros(dim))).float())
        self.ln_scale_factor = 1.0 / math.sqrt(layer_idx + 1) if ln_scale else 1.0

    def forward(self, x: Tensor, x0: Tensor, bigram_emb: Tensor | None = None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        if bigram_emb is not None:
            x = x + mix[2][None, None, :] * bigram_emb
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        bigram_emb = self.bigram(input_ids) if self.bigram is not None else None
        ...
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, x0, bigram_emb)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[self.num_encoder_layers + i](x, x0, bigram_emb)
