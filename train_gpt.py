...
class Block(nn.Module):
    def __init__(
        ...
        self.use_biglu = layer_idx < num_layers - 3
        ...
        if self.use_biglu:
            self.mlp = BigLU(dim, biglu_mult, bigram_vocab_size)
        else:
...
def main() -> None:
    ...
    # - bigram embedding (Adam) uses EMBED_LR (token-style optimizer path)
    # - untied lm_head (Adam) uses HEAD_LR
    # - matrix params in transformer blocks use MATRIX_LR via Muon
    # - vectors/scalars use SCALAR_LR via Adam
    block_named_params = list(base_model.blocks.named_parameters())
    bigram_params = [
        p for name, p in block_named_params if "bigram" in name and p.ndim == 2
    ]
    matrix_params = [
        p
